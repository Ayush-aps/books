...
router.get("/book/:id", ensureAuthenticated, ensureSeller, async (req, res) => {
  try {
    ...
    res.render("seller/book-details", {
...
router.get("/books", ensureAuthenticated, ensureSeller, async (req, res) => {
  try {
    ...
    // Build query - removed seller filter to show all books
    const query = {};
    ...
    res.render('seller/books', {
      title: 'Browse Books - Bookish Seller',
      books,
      genres,
      filters: {
        search,
...
/**
 * @route   GET /seller/view/:id
 * @desc    View book details
 * @access  Private (Seller)
 */
router.get("/view/:id", ensureAuthenticated, ensureSeller, async (req, res) => {
  try {
    const book = await Book.findById(req.params.id)
      .populate('seller', 'name email');
